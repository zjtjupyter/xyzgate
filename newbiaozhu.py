...
SCALE = 5 #batch divided into SCALE parts


# for dir_1 in list:
dir_1 = '1'    #choose file name
list_1 = os.listdir(rootdir + dir_1)        #./tagpic/1/   all name in the file
for A_flag in range(0,SCALE):
	json_save = {}        					#clear json_save dict to release cache
	for dir_2 in list_1:						#./tagpic/1/name
		...
		for s_num in range(int(A_flag*len(lines)/SCALE),int((A_flag+1)*len(lines)/SCALE)):
	...
	print('----------Part %d label data saved!------------'%(A_flag+1))
	...
	print('--------Part %d Finished!-------------'%(A_flag+1))
print('--------All Finished!-------------')
